...
- Search:
    - live search input
    - *BONUS* field selector (name, race, alignment, etc.) 
    - *BONUS* operator selector (include, exclude, >, <, =)
    - *BONUS* fuzzy search
...
- *BONUS* A slick design (CSS improvements for UI/UX)
...
- *BONUS* A slick design (CSS improvements for sorting UI)
...
- State Management:
    ...
    - *BONUS* read/write URL parameters

- *BONUS* A slick design (CSS improvements for table and layout)
...
- App Orchestration:
    ...
    - *BONUS* initial load (URL -> state -> render)
